...
def read_map_data(directory_path, date):
  # Loop through specified directory and retrieve data from those files
...
if __name__ == "__main__":
  ...
  file_path = "/data01/jyin/NOAA20VI"

  # Retrieve input for year to be retrieved from the command line
  year = str(input("Enter year of directory to be retrieved: "))
  date = str(input("Enter date of directory to be retrieved (YYYYMMDD): "))
  year_date = f"{year}/{date}"
  file_path = os.path.join(file_path, year_date)

  # Read the data
  read_map_data(file_path, date)
